...
function PageHeader() {
  ...
  const handleSubmit = (e) => {
    ...
    fetch("https://kitecast-stg-api.azurewebsites.net/api/v1/media/68db375a-64b8-4dcd-b5e5-31a4628ab453", {
      method: 'PATCH',
      body: formData,
    })
      .then((res) => res.json())
            ...
            <form onSubmit={handleSubmit}>
              ...
              <Box sx={{ mt: 3 }}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', px: 15, border: '2px dotted', borderColor: darkMode ? 'white' : 'black', color: darkMode ? 'white' : 'black', py: 4, mt: 2 }}>
                  ...
                  <Box>
                    <input
                      style={{ color: '#E44B23', borderColor: '#E44B23', }}
                      type="file"
                      name="myImage"
                      onChange={(event) => {
              ...
              <button type='submit' style={{cursor:'pointer', marginTop: "20px", background: '#E44B23', color: 'white', paddingTop: '10px', paddingBottom: '10px', paddingLeft: '25px', paddingRight: '25px', borderRadius: '10px' }}>Upload Media</button>
            </form>
          </Box>
        </Modal>
      </Box>
